    ...
    internal class PlagueArrowPROJ : ModProjectile, ILocalizedModType
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (ModContent.GetInstance<CREsConfigs>().EnableSpecialEffects)
            {
                ...
                int numParticles = hasWitherBlossom ? 20 : 7; // 如果存在 WitherBlossom，则生成更多粒子

                // 改为 PlagueTaintedDrone 的粒子特效，但保留原逻辑
                for (int i = 0; i < numParticles; i++)
                {
                    float speedFactor = hasWitherBlossom ? 5f + i * 0.4f : 3f + i * 0.2f; // 控制速度
                    int dustType = hasWitherBlossom ? DustID.GreenTorch : DustID.PinkTorch;

                    // 向上发射
                    Vector2 velocityUp = new Vector2(0, -speedFactor);
                    Dust dustUp = Dust.NewDustPerfect(Projectile.Center, dustType, velocityUp, 0, Color.Lime, 1.5f);
                    dustUp.noGravity = true;

                    // 向下发射
                    Vector2 velocityDown = new Vector2(0, speedFactor);
                    Dust dustDown = Dust.NewDustPerfect(Projectile.Center, dustType, velocityDown, 0, Color.Lime, 1.5f);
                ...
                for (int i = 0; i < numParticles; i++)
                {
                    ...
                    Dust dustLeft = Dust.NewDustPerfect(Projectile.Center, dustType, velocityLeft, 0, Color.Lime, 1.5f);
                    dustLeft.noGravity = true;

                    // 向右发射
                    Vector2 velocityRight = new Vector2(speedFactor, 0);
                    Dust dustRight = Dust.NewDustPerfect(Projectile.Center, dustType, velocityRight, 0, Color.Lime, 1.5f);
