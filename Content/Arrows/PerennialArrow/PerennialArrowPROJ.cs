...
namespace FKsCRE.Content.Arrows.PerennialArrow
{
    public class PerennialArrowPROJ : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            //// 在敌人身上放置PerennialArrowFlower
            //NPC target = Main.npc[(int)Projectile.ai[0]];
            //if (target.active && !target.HasBuff(ModContent.BuffType<PerennialArrowEBuff>()))
            //{
            //    Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<PerennialArrowFlower>(), 0, 0, Projectile.owner);
            //}
        }
        ...
        //    // 如果敌人没有增伤Buff，则放置花
        //    if (!target.HasBuff(ModContent.BuffType<PerennialArrowEBuff>()))
        //    {
        //        // 在击中的敌人身上放置PerennialArrowFlower
        //        Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, Vector2.Zero, ModContent.ProjectileType<PerennialArrowFlower>(), 0, 0, Projectile.owner);
        //    }
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            //// 根据造成的伤害吸取生命值
            //int damageDealt = (int)modifiers.FinalDamage.Base; // 将 float 显式转换为 int
            //int lifeSteal = Main.rand.Next(5, 51); // 生成5到50的随机生命吸取量

            //// 吸取的生命值不能超过造成伤害的值
            //if (lifeSteal > damageDealt)
            //{
            //    lifeSteal = damageDealt;
            //}

            //// 为玩家回复生命
            //Main.player[Projectile.owner].statLife += lifeSteal;
