...
namespace FKsCRE.Content.Ammunition.CPreMoodLord.PlagueBullet
{
    internal class PlagueBulletPROJ : ModProjectile, ILocalizedModType
    {
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            {
                modifiers.FinalDamage *= 2.2f; // 增加伤害
            }
            else
            {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (ModContent.GetInstance<CREsConfigs>().EnableSpecialEffects)
            {
                ...
                {
                    float angle = MathHelper.TwoPi / 3 * (i % 3) + Main.rand.NextFloat(-0.1f, 0.1f); // 三角形方向随机微调
                    Vector2 velocity = new Vector2(velocityMultiplier, 0).RotatedBy(angle) * Main.rand.NextFloat(0.8f, 1.2f);
                    Dust dust = Dust.NewDustPerfect(Projectile.Center, DustID.GreenTorch, velocity, 0, Color.Lime, 1.5f);
                    dust.noGravity = true;
